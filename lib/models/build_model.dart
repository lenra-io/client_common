...
class BuildModel extends ChangeNotifier {
  ...
  Future<List<BuildResponse>> fetchBuilds(int appId) async {
    var res = await Status().handle(() => ApplicationApi.getBuilds(appId), notifyListeners);
  ...
  Future<BuildResponse> createBuild(int appId) async {
    var res = await Status().handle(() => ApplicationApi.createBuild(appId, CreateBuildRequest()), notifyListeners);
